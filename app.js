...
import {
  state, setState, activeMonthId, setActiveMonthId,
  MONTH_NAMES, getActiveMonth, readLocalCache, writeLocalCache,
  clearSession, getDisplayName, cacheDisplayName,
} from "./state.js";
import {
  loadState, dbAddMonth, dbUpdateBudget, dbDeleteMonth,
  dbAddExpense, dbUpdateExpense, dbDeleteExpense,
  dbUpdateDisplayName, dbUpdatePassword,
} from "./api.js";
import {
  registerAuthCallbacks, handleSessionInvalid,
  doLogin, signInWithPassword, signOut, getSession,
} from "./auth.js";
import {
  showScreen, applyTheme, getPreferredTheme, toggleTheme,
  showFieldError, clearFieldError, showBanner, clearBanner,
  renderWelcomeName, sortMonths, renderHistory,
  openMonth, renderStats, renderExpenses,
  showUndoToast, hideUndoToast,
  openEditExpenseModal, closeEditExpenseModal, getEditingExpenseId,
  openDeleteMonthModal, closeDeleteMonthModal, getPendingDeleteMonthId,
  openMonthPicker, closeMonthPicker,
  profileOverlay, openProfileModal, closeProfileModal,
  ensureCurrentMonth, registerDeleteExpenseCb,
} from "./ui.js";

// -- WIRE AUTH CALLBACKS (breaks circular dep)
registerAuthCallbacks({
  showScreen, showBanner, renderHistory,
  renderWelcomeName, ensureCurrentMonth: () => ensureCurrentMonth(dbAddMonth), sortMonths,
});
...
function undoDeleteExpense() {
  ...
  if (m.id === activeMonthId) { renderStats(m); renderExpenses(m); }
  hideUndoToast();
}
...
document.getElementById("btn-pick-confirm").addEventListener("click", async () => {
  ...
  const newId = await dbAddMonth(y, m + 1, name, () => handleSessionInvalid("SESSION_INVALID"));
  if (!newId) return;
  state.months.push({ id: newId, name, year: y, month: m, budget: null, expenses: [] });
  sortMonths();
  closeMonthPicker();
  renderHistory();
  openMonth(newId);
...
document.addEventListener("keydown", (e) => {
  ...
  if (pickerEl && !pickerEl.classList.contains("hidden")) { closeMonthPicker(); return; }
...
btnSetBudget.addEventListener("click", async () => {
  ...
  if (!ok) { showBanner("login-error", "Could not save budget. Try again."); return; }
  m.budget = val;
...
async function addExpense() {
  ...
  if (!dateVal) {
    ...
    expDateInput.classList.remove("is-invalid");
    if ((expDateInput.min && dateVal < expDateInput.min) || (expDateInput.max && dateVal > expDateInput.max)) {
      showFieldError(expDateInput, "err-exp-date", "Date is outside this month.");
    } else { clearFieldError(expDateInput, "err-exp-date"); }
  }
  if (!valid) return;
  const row = await dbAddExpense(m.id, desc, val, dateVal);
  if (!row) { showBanner("login-error", "Could not save expense. Try again."); return; }
  m.expenses.push({ id: row.id, desc, val, date: dateVal, createdAt: row.created_at });
  renderStats(m); renderExpenses(m);
...
document.getElementById("btn-edit-expense-save").addEventListener("click", async () => {
  ...
  let valid = true;
  if (!newDesc) { showFieldError(descEl, "err-edit-exp-desc", "Description is required."); valid = false; }
  else { clearFieldError(descEl, "err-edit-exp-desc"); }
  if (!newVal || newVal <= 0) { showFieldError(valEl, "err-edit-exp-val", "Enter a valid amount."); valid = false; }
  else { clearFieldError(valEl, "err-edit-exp-val"); }
  if (!newDate) { showFieldError(dateEl, "err-edit-exp-date", "Date is required."); valid = false; }
  else { clearFieldError(dateEl, "err-edit-exp-date"); }
  if (!valid) return;
  const ok = await dbUpdateExpense(exp.id, newDesc, newVal, newDate);
  if (!ok) { showBanner("login-error", "Could not update expense. Try again."); return; }
  exp.desc = newDesc; exp.val = newVal; exp.date = newDate;
  renderStats(m); renderExpenses(m);
...
document.getElementById("btn-delete-month-confirm").addEventListener("click", async () => {
  const id = getPendingDeleteMonthId();
  if (!id) return;
  const ok = await dbDeleteMonth(id);
  if (!ok) { showBanner("login-error", "Could not delete month. Try again."); closeDeleteMonthModal(); return; }
  setState({ months: state.months.filter((x) => x.id !== id), displayName: state.displayName });
  closeDeleteMonthModal(); renderHistory();
});
...
document.getElementById("btn-profile-save").addEventListener("click", async () => {
  ...
  if (newName) {
    ...
    cacheDisplayName(newName); state.displayName = newName;
  ...
  if (currentPass || newPass || confirmPass) {
    ...
    if (!currentPass) { showFieldError(cpEl, "err-current-pass", "Current password is required."); return; }
    if (newPass !== confirmPass) { showFieldError(cfEl, "err-confirm-pass", "Passwords do not match."); return; }
    if (newPass.length < 8) { showFieldError(npEl, "err-new-pass", "Minimum 8 characters."); return; }
    btn.classList.add("btn--loading"); btn.disabled = true;
    try {
      const result = await dbUpdatePassword(newPass);
      if (!result.ok) { showFieldError(cpEl, "err-current-pass", "Could not update password. Try again."); return; }
      ...
      if (sEl) { sEl.style.color = "var(--success)"; sEl.textContent = "Password updated."; setTimeout(() => { sEl.textContent = ""; sEl.style.color = ""; }, 3000); }
...
document.getElementById("btn-logout").addEventListener("click", async () => {
  ...
  try {
    await signOut(); closeProfileModal(); clearSession();
    setState({ displayName: null, months: [] }); setActiveMonthId(null);
...
document.getElementById("btn-login").addEventListener("click", async () => {
  ...
  clearFieldError(uEl, "err-username"); clearFieldError(pEl, "err-password"); clearBanner("login-error");
  let valid = true;
  ...
  btnLogin.classList.add("btn--loading"); btnLogin.disabled = true;
  try {
    ...
    showBanner("login-error", "Could not reach the server. Check your connection.");
  } finally { btnLogin.classList.remove("btn--loading"); btnLogin.disabled = false; }
});
